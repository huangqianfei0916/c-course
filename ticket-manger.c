...
void cancelbook()
{
    ...
    case 2:
        s[i].second_seat += s[i].book->next->bookednum;

        save_flight();
        break;

    case 3:
        s[i].third_seat += s[i].book->next->bookednum;
    ...
    p1 = s[i].afterqueue->next;
    pre1 = s[i].afterqueue;

    printf("%s\t%d\t%d\t%d", p1->name, p1->afternum, p1, n);
    system("pause");
